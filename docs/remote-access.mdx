...
EchOS's Docker entrypoint automatically fixes data directory ownership on every startup — it runs briefly as root, corrects any ownership mismatches, then drops to the `node` user before starting the app. On most setups no manual steps are needed.
