...
You can also use `/save` at any time to re-export the last AI response as a `.md` file. The stored response is scoped per chat — a response in a private chat is not accessible via `/save` in a group chat. Stored responses expire after 30 minutes and are cleared on `/reset`.
