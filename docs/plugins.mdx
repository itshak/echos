...
**Features:**
- Download PDFs from any URL and extract text via `pdf-parse` (pure JS, no native deps)
- Preserves page count, author, and source URL as metadata
- Truncates oversized content gracefully (max 500 000 chars) with a `[content truncated]` notice
...
**Features:**
- Downloads audio from any URL and transcribes via `whisper-1`
